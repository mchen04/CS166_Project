...
-- ucr server version: loads csv from $PGDATA (copy files there first)
...
-- load csv data from $PGDATA (must run: cp data/*.csv $PGDATA/ first)
...
COPY Owns FROM 'owns.csv' DELIMITER ',' CSV HEADER;
COPY Service_Request FROM 'service_request.csv' DELIMITER ',' CSV HEADER;
COPY Closed_Request FROM 'closed_request.csv' DELIMITER ',' CSV HEADER;
